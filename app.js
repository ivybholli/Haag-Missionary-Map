...
function createMarkerIcon(group){const count=group.length,hasMale=group.some(m=>getSex(m)!=="Female"),hasFemale=group.some(m=>getSex(m)==="Female"),hasCurrent=group.some(isCurrentlyServing),hasInLaw=group.some(isInLaw);let color=getColor(getSex(group[0]));if(hasMale&&hasFemale)color="#7c3aed";let inner="";if(count>1)inner=`<span class="marker-number">${count}</span>`;else if(hasCurrent)inner=`<span class="marker-check">✓</span>`;const cls=["mission-marker",count>1?"duplicate-marker":"",hasInLaw&&count===1?"heart-marker":"dot-marker"].join(" ");const wrapper=document.createElement("div");wrapper.className="custom-marker";wrapper.innerHTML=`<div class="${cls}" style="--marker-color:${color}">${inner}</div>`;return wrapper;}
