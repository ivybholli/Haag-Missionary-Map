...
function preferredMissionaryForSearch(value){const q=normalize(value);if(!q)return null;const exactNameMatches=allMissionaries.filter(m=>normalize(getName(m))===q&&m.coords);if(exactNameMatches.length){return exactNameMatches.find(isOriginalAssignment)||exactNameMatches[0];}const filtered=getFilteredData().filter(m=>m.coords);const names=new Set(filtered.map(m=>normalize(getName(m))).filter(Boolean));if(filtered.length===1)return filtered[0];if(names.size===1&&filtered.length){return filtered.find(isOriginalAssignment)||filtered[0];}return null;}
...
function zoomToCountry(country){const items=getFilteredData().filter(m=>getCountry(m)===country&&m.coords);if(!items.length)return;hideDetailPreview();if(currentPopup){currentPopup.remove();currentPopup=null;}if(items.length===1){const c=items[0].coords;map.flyTo({center:[c.lng,c.lat],zoom:Math.max(map.getZoom(),5),duration:1200,essential:true});return;}let bounds=new maplibregl.LngLatBounds([items[0].coords.lng,items[0].coords.lat],[items[0].coords.lng,items[0].coords.lat]);items.slice(1).forEach(m=>bounds.extend([m.coords.lng,m.coords.lat]));map.fitBounds(bounds,{padding:{top:90,bottom:90,left:90,right:90},maxZoom:5,duration:1200});}
...
function setupControls(){
  ...
  searchInput.addEventListener("input",e=>{activeSearch=e.target.value;drawMarkers();navigateSearchResult({clearSearch:true});});
