...
function geocodeCacheKey(m){
  return normalize([getMissionName(m), getCity(m), getState(m), getCountry(m)].join("|"));
}
...
async function geocodeMissingMissionary(m){
  ...
  for(const q of buildGeocodeQueries(m)){
    ...
    try {
      ...
      console.warn("Could not find coordinates for:", q);
    }
  }
...
async function fillMissingCoordinatesInBackground(){
  ...
  for(const m of missing){
    ...
    if(coords){
      ...
      console.warn("Still missing coordinates for:", getMissionName(m));
    }
  }
}
...
async function buildMap(){
  ...
  try {
    ...
    console.warn("Coordinates sheet could not be loaded.");
  }
  ...
  coordinates.forEach(r => {
    ...
    if(mission && !isNaN(lat) && !isNaN(lng)){
      lookup[normalize(mission)] = { lat, lng };
    }
  });
